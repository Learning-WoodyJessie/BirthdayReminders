    ...
    <>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/contacts/new" className="btn-primary text-white font-semibold px-4 py-2 rounded-xl text-sm">
            + Add contact
          </Link>
        </div>
        ...
        {allContacts.length === 0 ? (
          <div className="glass rounded-2xl p-12 text-center">
            ...
            <h2 className="text-white font-bold text-xl mb-2">No contacts yet</h2>
            <p className="text-white/40 text-sm mb-6">Add the people you care about and never miss their special days.</p>
          ...
          <div className="glass rounded-2xl overflow-hidden">
            {allContacts.map((contact) => (
              <div key={contact.id} className="flex items-center justify-between px-5 py-4 border-b border-white/5 last:border-0">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-white/40 text-xs capitalize">{contact.relationship}</p>
                  </div>
                </div>
                <div className="hidden sm:flex items-center gap-8">
                  <div className="text-right">
                    <p className="text-white/30 text-xs mb-0.5">Birthday</p>
                    <p className="text-white/70 text-xs">{formatDate(contact.birthday)}</p>
                  </div>
                  {contact.anniversary && (
                    <div className="text-right">
                      <p className="text-white/30 text-xs mb-0.5">Anniversary</p>
                      <p className="text-white/70 text-xs">{formatDate(contact.anniversary)}</p>
                    </div>
                  )}
                </div>
              </div>
